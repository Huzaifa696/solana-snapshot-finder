...
def main_worker():
    try:
        ...
        for i, rpc_node in enumerate(json_data["rpc_nodes"], start=1):
            ...
            except Exception as speedErr:
                logger.error(f'Exception in measure_speed() func\n{speedErr}')
                continue

            try:
                down_speed_mb = convert_size(down_speed_bytes)
            except Exception as sizeErr:
                logger.error(f'Exception in convert_size() func\n{sizeErr}')
                continue
        ...
        logger.error(f'Exception in main_worker() func\n{unknwErr}')
        return 1
